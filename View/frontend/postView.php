...
<div class="containerComment col-md-6 m-2">
     <form class="clearfix" method="post" action="index.php?action=alertcomment&amp;commentId=<?= $comment['id'] ?>&amp;postId=<?= $post['id'] ?>">
       <span class="font-weight-bold"><?= strip_tags($comment['author']); ?></span><span class="font-weight-lighter font-italic"> le <?= $comment['comment_date_fr']; ?></span>
     <button type="submit" class="btn float-right" name="alertTrue"  value="true"> <span class="alert-comment" title="Signaler le commentaire"><i class="far fa-bell animated infinite tada"></i><span> </button>
   </form>
